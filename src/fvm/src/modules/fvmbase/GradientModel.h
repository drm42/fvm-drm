...
public:
  ...
  static
  shared_ptr<GradientMatrixBase>
  getLeastSquaresInteriorGradientMatrix3D(const Mesh& mesh, const GeomFields& geomFields)
  {
    const StorageSite& cells = mesh.getCells();
    const StorageSite& faces = mesh.getInteriorFaceGroup().site;
  ...
  static GradMatrixType&
  getInteriorGradientMatrix(const Mesh& mesh, const GeomFields& geomFields)
  {
    if (_gradientMatricesMap.find(&mesh) == _gradientMatricesMap.end())
    {
        _gradientMatricesMap[&mesh] = getLeastSquaresInteriorGradientMatrix3D(mesh,geomFields);
    }
    return dynamic_cast<GradMatrixType&>(*_gradientMatricesMap[&mesh]);
  }
  ...
  void computeInterior()
  {
	...
	if (mesh.isShell() == false){
        ...
        GradMatrixType& gradMatrix = getInteriorGradientMatrix(mesh,_geomFields);
