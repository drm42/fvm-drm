...
public:
  ...
  void init()
  {
	...
	*oneCell = T(1.0);
	...
	//previous timestep specific heat
	shared_ptr<TArray> cpN1(new TArray(cells.getCount()));
	*cpN1 = vc["density"] * vc["specificHeat"];
	_thermalFields.specificHeatN1.addArray(cells, cpN1);
  ...
  void linearize(LinearSystem& ls)
  {
	...
	if (_options.enthalpyModel)
	  ...
	  {
	    shared_ptr<Discretization>
	      td(new TimeDerivativeDiscretizationVarDen<T, T, T>
		 (_meshes, _geomFields, 
		  _thermalFields.temperature, 
		  _thermalFields.temperatureN1,
		  _thermalFields.temperatureN2,
		  _thermalFields.specificHeat,
		  _thermalFields.specificHeatN1,
		  _options["timeStep"]));
  ...
  void updateTime()
  {
    ...
    for (int n=0; n<numMeshes; n++)    {
      ...
      TArray& specificHeatN1 =
          dynamic_cast<TArray&>(_thermalFields.specificHeatN1[cells]);
      if (_options.enthalpyModel) {
	TArray& enthalpy =
